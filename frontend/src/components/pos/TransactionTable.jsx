...
const TransactionTable = ({ cart, onUpdateQty, onRemove, products, onAddItem }) => {
    ...
    const handleNameSubmit = (e) => {
        ...
        if (e.key === 'ArrowDown') {
            ...
            e.preventDefault();
            const nextIndex = selectedIndex <= 0 ? filteredSuggestions.length - 1 : selectedIndex - 1;
            setSelectedIndex(nextIndex);
            setTimeout(() => {
                document.getElementById('suggestion-' + nextIndex)?.scrollIntoView({ block: 'nearest' });
            }, 0);
                        ...
                        {filteredSuggestions.length > 0 ? (
                            filteredSuggestions.map((p, idx) => (
                                <div
                                    key={p.id}
                                    id={`suggestion-${idx}`}
                                    className={`flex text-xs font-mono cursor-pointer border-b border-gray-200 hover:bg-blue-600 hover:text-white ${idx === selectedIndex || (selectedIndex === -1 && idx === 0) ? 'bg-red-900 text-white font-bold' : 'text-gray-800 bg-gray-50'}`}
